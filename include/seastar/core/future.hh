...
public:
    ...
    friend class future;
    template <typename T> friend struct futurize;
};
...
protected:
    ...
    promise_base(future_state_base* state) noexcept : _state(state) {
        ...
        internal::trace_runtime_edge(internal::get_current_traced_ptr(), this, true);
    }
...
#ifdef SEASTAR_DEBUG
        if (_state.available()) {
            internal::trace_runtime_edge(this, tws);
...
private:
    ...
    Result then_impl_nrvo(Func&& func) noexcept {
        ...
        schedule(fut.get_promise(), std::move(func), [](pr_type&& pr, Func& func, future_state&& state) {
            if (state.failed()) {
                internal::trace_runtime_edge(internal::get_current_traced_ptr(), &pr);
    ...
    then_impl(Func&& func) noexcept {
        ...
        {
            internal::update_current_traced_vertex update(this);
            using futurator = futurize<internal::future_result_t<Func, T SEASTAR_ELLIPSIS>>;
            if (failed()) {
...
private:
    ...
    void forward_to(internal::promise_base_with_type<T SEASTAR_ELLIPSIS>&& pr) noexcept {
        if (_state.available()) {
            internal::trace_runtime_edge(this, &pr);
...
private:
    void set_callback(continuation_base<T SEASTAR_ELLIPSIS>* callback) noexcept {
        if (_state.available()) {
            internal::trace_runtime_edge(this, callback);
